...
async def video_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not db_user.get('is_premium', False) and today_uploads >= Config.DAILY_LIMIT:
        await message.reply_text(f"You've reached your daily upload limit ({Config.DAILY_LIMIT}). Upgrade to premium for unlimited uploads.")
        return
    ...
    keyboard = [[InlineKeyboardButton(ch['channel_name'], callback_data=f"upload_channel_{str(ch['_id'])}")] for ch in channels]
